 ...
 */
async function findService(relativePathToApp, instanceName) {
    ...
    const serviceFileName = path.basename(absolutePath), serviceFileNameWithInstance = serviceFileName.replace(/\@/, `@${instanceName}`);
    multi_level_logger_1.Trace({ serviceFileName, serviceFileNameWithInstance });
